...
export default function GoogleSignInButton({ label = "Continue with Google" }: Props) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className="w-full py-3 flex items-center justify-center gap-3 border border-border rounded-xl bg-background hover:bg-muted/50 transition-colors text-sm font-medium text-foreground disabled:opacity-50"
    >
      {loading ? (
