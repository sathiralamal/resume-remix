...
- **AI SDKs:** Google Generative AI (`@google/generative-ai`), OpenAI (`openai`), Anthropic (`@anthropic-ai/sdk`), Groq (`groq-sdk`), OpenRouter (`@openrouter/sdk`)
...
- Providers are selected at runtime using the `AI_PROVIDER` environment variable (`gemini`, `openai`, `anthropic`, `groq`, `openrouter`).
